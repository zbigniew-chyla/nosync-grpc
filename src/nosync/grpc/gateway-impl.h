...
inline gateway::gateway(os::mt_event_loop &evloop, std::shared_ptr<::grpc::ChannelInterface> channel)
    ...
    collector_handle()
{
    ...
    auto collector_thread = std::thread(
        [res_handlers_executor = evloop.get_mt_executor(), cqueue = *cqueue_ptr]() {
            while (true) {
...
inline std::shared_ptr<gateway> make_gateway(
    os::mt_event_loop &evloop, std::shared_ptr<::grpc::ChannelInterface> channel)
{
    class gateway_impl : public gateway
    {
    public:
        gateway_impl(os::mt_event_loop &evloop, std::shared_ptr<::grpc::ChannelInterface> &&channel)
            : gateway(evloop, std::move(channel))
        {
        }
    };

    return std::make_shared<gateway_impl>(evloop, std::move(channel));
}
